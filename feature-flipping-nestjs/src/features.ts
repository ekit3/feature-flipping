...
let Criteria = [
  ...
  },
  {
    ...
    check: function (user, objType) {
      switch (objType) {
        case 'us':
          return user.role === 'scrum';
        case 'ts':
          return (
            user.role === 'po' || user.role === 'scrum' || user.role === 'dev'
          );
        default:
          return false;
      }
...
let Features = [
  ...
  },
  {
    ...
    description:
      'Experimental feature still in development, useful for internal development', // user-defined
